...
public class CustomPagedResourceAssembler<T> extends PagedResourcesAssembler<T> {
 ...
 public CustomPagedResourceAssembler(HateoasPageableHandlerMethodArgumentResolver resolver, @Nullable Integer pageSize) {
   super(resolver, null);
  ...
  private <R> PagedModel<R> addPaginationLinks(PagedModel<R> resources, Page<?> page, Optional<Link> link) {
    ...
    int baseIndex = (currentIndex / this.PAGE_SIZE) * this.PAGE_SIZE;
